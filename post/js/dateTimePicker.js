...
var CalPosOffsetY = 30; //Y position offset relative to calendar icon, can be negative value
...
var StartYear = 2000; //First selectable year
...
var imageFilesPath = "images/";//"jdtp-images/"; //Path of date time picker image files relative to HTML page.
...
function RenderCssCal(bNewCal)
{
	...
	if (Cal.ShowTime === true)
	{
		...
		vCalTime += "<td width='22px;'><input type='text' name='hour' maxlength=2 size=1 style=\"WIDTH:22px;font-size:10px;\" value=" + showHour + " onkeyup=\"javascript:Cal.SetHour(this.value)\">";
		vCalTime += "</td><td style='font-weight:bold;text-align:center;'>:</td><td width='22px'>";
		vCalTime += "<input type='text' name='minute' maxlength=2 size=1 style=\"WIDTH: 22px;font-size:10px;\" value=" + Cal.Minutes + " onkeyup=\"javascript:Cal.SetMinute(this.value)\">";
	...
	else
	{
		...
		if (bNewCal === true)
		{
			winCal.style.right = (0) + 'px';
			winCal.style.top = (ypos - CalPosOffsetY)+50 + 'px';
			//alert((ypos - CalPosOffsetY)+10 + 'px');
		}
	}
...
function NewCssCal(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pShowSeconds)
{
	...
	/*else
	{
		exDateTime = document.getElementById(etril).value; //Existing Date Time value in textbox.
	}*/
	if (exDateTime)
