...
class ajax_table {
  ...
  function getRecords(){
	$this->res = mysql_query("select * from worker");
  ...
  function save($data){
	if(count($data)){
		...
		mysql_query("insert into worker (".implode(",",array_keys($data)).") values ('".$values."')");
  ...
  function delete_record($id){
	 if($id){
		mysql_query("delete from worker where id = $id limit 1");
  ...
  function update_record($data){
	if(count($data)){
		...
		$str = substr($str,0,-1);
		$sql = "update worker set $str where id = $id limit 1";
  ...
  function update_column($data){
	if(count($data)){
		$id = $data['rid'];
		unset($data['rid']);
		$sql = "update worker set ".key($data)."='".$data[key($data)]."' where id = $id limit 1";
		$res = mysql_query($sql);
		if(mysql_affected_rows()) return $id;
		return 0;
